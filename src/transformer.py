...
class HyperGraph(nn.Module):
    def __init__(self, forward_edge, backward_edge, num_hyperedges, num_nodes, dim, use_hypergraph, num_layers, heads, dropout, activation):
        super(HyperGraph, self).__init__()
        self.layers0 = nn.ModuleList()
        self.layers1 = nn.ModuleList()

        # self.layers2 = nn.ModuleList()  # 新增的从节点到节点的层
        ...
        for _ in range(num_layers):
            ...
            # self.layers2.append(GATv2Conv(dim, dim // heads, heads=heads, dropout=dropout))
            # self.layers3.append(GATv2Conv(dim, dim // heads, heads=heads, dropout=dropout))
        ...
        #1
        # self.node_embedding = nn.parameter.Parameter(torch.randn(num_nodes, dim) * 0.1)
        # self.hyperedge_embedding = nn.parameter.Parameter(torch.randn(num_hyperedges, dim) * 0.1)

        # 2
        # 初始化节点嵌入向量
        # self.node_embedding = nn.parameter.Parameter(torch.rand(num_nodes, dim) * 0.2 - 0.1)
        #
        # # 初始化超边嵌入向量
        # self.hyperedge_embedding = nn.parameter.Parameter(torch.rand(num_hyperedges, dim) * 0.2 - 0.1)
    ...
    def forward(self):
        node_embedding = self.node_embedding
        # 增加残差连接
        residual = node_embedding
        if self.use_hypergraph is True:
            hyperedge_embedding = self.hyperedge_embedding.to(node_embedding.device)
            for i in range(self.num_layers):
                tmp = self.layers0[i]((node_embedding, hyperedge_embedding),
                                      self.forward_edge.to(node_embedding.device))
                tmp = self.activate(tmp)
                hyperedge_embedding = hyperedge_embedding + tmp

                tmp = self.layers1[i]((hyperedge_embedding, node_embedding),
                                      self.backward_edge.to(node_embedding.device))
                tmp = self.activate(tmp)
                node_embedding = node_embedding + tmp + residual
                residual = node_embedding

                # 新增的从节点到节点的信息传递
                # tmp = self.layers2[i]((node_embedding, node_embedding), self.forward_edge.to(node_embedding.device))
                # tmp = self.activate(tmp)
                # node_embedding = node_embedding + tmp + residual
                # residual = node_embedding

                # tmp = self.layers3[i]((node_embedding, node_embedding), self.backward_edge.to(node_embedding.device))
                # tmp = self.activate(tmp)
                # node_embedding = node_embedding + tmp + residual
                # residual = node_embedding

                # 新增的从超边到超边的信息传递
                # tmp = self.layers3[i]((hyperedge_embedding, hyperedge_embedding), self.backward_edge)
                # tmp = self.activate(tmp)
                # hyperedge_embedding = hyperedge_embedding + tmp
...
class TransformerLayer(nn.Module):
    def __init__(self, hidden_dim: int, heads: int, dropout_prob: float, activation: str, use_edge: bool, use_attention: bool, remove_mask: bool, use_node: bool, bias=True, times=2) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, graph: torch.Tensor, edge_key: torch.Tensor, edge_value: torch.Tensor, edge_query: torch.Tensor):
        # if self.use_attention is True:
        #     attn = self.attention(query=x, key=x, value=x, graph=graph, edge_key=edge_key, edge_value=edge_value,
...
class Transformer(nn.Module):
    def __init__(self, graph, vocab_size: int, trans_layers: int, hyper_layers: int, hidden_dim: int,
            trans_heads: int, hyper_heads: int, use_hypergraph: bool, trans_dropout: float, hyper_dropout: float,
            decoder_activation: str, hyper_activation: str, use_edge: bool, remove_mask: bool, use_node: bool, bias=True, times=2) -> None:
        super().__init__()
        self.hidden_dim = hidden_dim
        self.layers = nn.ModuleList()
        for _ in range(trans_layers):
            self.layers.append(TransformerLayer(hidden_dim, trans_heads, trans_dropout, decoder_activation, use_edge, remove_mask, use_node, bias, times=times))
        self.input_norm = nn.LayerNorm(hidden_dim)
        self.input_dropout = nn.Dropout(p=trans_dropout)
        ...
        self.edge_query_embedding = nn.Embedding(14, hidden_dim // trans_heads, padding_idx=0)
        self.edge_key_embedding = nn.Embedding(14, hidden_dim // trans_heads, padding_idx=0)
        self.edge_value_embedding = nn.Embedding(14, hidden_dim // trans_heads, padding_idx=0)
        self.hype = HyperGraph(*graph, vocab_size, hidden_dim, use_hypergraph, hyper_layers, hyper_heads, hyper_dropout, hyper_activation)
        self.init_params()
    ...
    def forward(self, input_ids, input_mask, mask_position, mask_output, edge_labels):
        embedding = self.hype().to(input_ids.device)
        ...
        return y    # y(batch_size, vocab_size)
